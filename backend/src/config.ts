...
function requireProductionEmailConfig() {
  if ((process.env.NODE_ENV === "production" || process.env.VERCEL) && !process.env.RESEND_API_KEY) {
    throw new Error("Email delivery is not configured. Set RESEND_API_KEY and RESEND_FROM_EMAIL server-side.");
  }
}
